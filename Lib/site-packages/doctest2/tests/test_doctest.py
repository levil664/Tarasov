...
required; the other attributes all have default values:
    ...
    >>> (example.source, example.want,
    ...  example.lineno, example.indent, example.options)
    ('print(1)\n', '1\n', 0, 0, {})

The first two attributes (`source` and `want`) may be
specified positionally; the remaining arguments should be specified as
keyword arguments:

    >>> example = doctest.Example('[1].pop()', '1',
    ...                           lineno=5, indent=4,
...
def test_python_ExpectedOutput(): r"""
Side note: the old exc_msg attribute of Example was absorbed into
doctest2.languages.python.ExpectedOutput.

    >>> from doctest2.languages.python import ExpectedOutput

Its constructor normalizes the `exc_msg` string to end in a newline,
unless it's `None`:

    Message spans one line
    >>> e = ExpectedOutput('', 'IndexError: pop from an empty list')
    >>> e.exc_msg
...
We'll simulate a __file__ attr that ends in pyc:
    ...
    >>> # The below alterations force doctest2's tests instead of the stdlib's
...
functions, classes, and the `__test__` dictionary, if it exists:
    ...
    >>> finder = doctest.DocTestFinder()
    >>> # Use module=tests.test_doctest, to prevent doctest from
...
will only be generated for it once:
    ...
    >>> from doctest2.tests import doctest_aliases
    ...
    2
    >>> print(tests[0].name)
    doctest2.tests.doctest_aliases.TwoNames
...
signaling an expected exception, and that will still be checked.
However, if the output prior to the exception does not match, then
the test will fail.
...
That is now corrected, and only real comments count as directives.

In older versions of doctest, the below would fail, because
DONT_ACCEPT_BLANKLINE would be set, even though it wasn't a real directive.

    >>> def printsomething(f): print('')
    >>> @printsomething
    ... def foo():
    ...     '''This is a pointless function with a comment that isn't a comment
    ...     
    ...     Example of a doctest containing a doctest! This is probably
    ...     dangerous.
    ...     
...
source:
    ...
    >>> s = '>>> # doctest: +ELLIPSIS'
    >>> # the following line contains doctest2-specific modifications:
    >>> test = doctest.DocTestParser().get_doctest(s, {}, 's', 's.py', 0) # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
...
words and expected output are converted to comments:
    ...
    >>> from doctest2.tests import test_doctest
    >>> name = 'doctest2.tests.test_doctest.sample_func'
    ...
    >>> name = 'doctest2.tests.test_doctest.SampleNewStyleClass'
    >>> print(doctest.testsource(test_doctest, name))
    print('1\n2\n3')
    # Expected:
    ## 1
    ...
    >>> name = 'doctest2.tests.test_doctest.SampleClass.a_classmethod'
    >>> print(doctest.testsource(test_doctest, name))
    print(SampleClass.a_classmethod(10))
    # Expected:
    ## 12
...
def test_pdb_set_trace():
    ...
    captures our debugger input:
      ...
      --Return--
      > <doctest doctest2.tests.test_doctest.test_pdb_set_trace[8]>(3)calls_set_trace()->None
      -> import pdb; pdb.set_trace()
      (Pdb) print(y)
      2
      (Pdb) up
...
def test_pdb_set_trace_nested():
    ...
    > <doctest doctest2.tests.test_doctest.test_pdb_set_trace_nested[0]>(5)calls_set_trace()
    -> self.f1()
    (Pdb) print(y)
    1
    (Pdb) step
    --Call--
    > <doctest doctest2.tests.test_doctest.test_pdb_set_trace_nested[0]>(7)f1()
    -> def f1(self):
    (Pdb) step
    > <doctest doctest2.tests.test_doctest.test_pdb_set_trace_nested[0]>(8)f1()
    -> x = 1
    (Pdb) step
    > <doctest doctest2.tests.test_doctest.test_pdb_set_trace_nested[0]>(9)f1()
    -> self.f2()
    (Pdb) step
    --Call--
    > <doctest doctest2.tests.test_doctest.test_pdb_set_trace_nested[0]>(11)f2()
    -> def f2(self):
    (Pdb) step
    > <doctest doctest2.tests.test_doctest.test_pdb_set_trace_nested[0]>(12)f2()
    -> z = 1
    (Pdb) step
    > <doctest doctest2.tests.test_doctest.test_pdb_set_trace_nested[0]>(13)f2()
    -> z = 2
    (Pdb) print(z)
    1
    (Pdb) up
...
def test_DocTestSuite():
    """DocTestSuite creates a unittest test suite from a doctest.
       ...
       by passing a module object:
         ...
         >>> import unittest
         >>> # doctest2-specific modification below:
         >>> import doctest2.tests.sample_doctest
         >>> suite = doctest.DocTestSuite(doctest2.tests.sample_doctest)
       ...
       We can also supply the module by name:
         ...
         >>> # doctest2-specific modification below:
         >>> suite = doctest.DocTestSuite('tests.sample_doctest')
         >>> suite.run(unittest.TestResult())
         <unittest.result.TestResult run=9 errors=0 failures=4>
       ...
       We can use the current module:
         ...
         >>> # doctest2-specific modification below:
         >>> suite = doctest2.tests.sample_doctest.test_suite()
         >>> suite.run(unittest.TestResult())
         <unittest.result.TestResult run=9 errors=0 failures=4>
       ...
       globals, triggering an extra error:
         ...
         >>> # doctest2-specific modification below:
         >>> suite = doctest.DocTestSuite('tests.sample_doctest', globs={})
         >>> suite.run(unittest.TestResult())
       ...
       we'll use the setUp function to supply the missing variable y:
         ...
         >>> # doctest2-specific modification below:
         >>> suite = doctest.DocTestSuite('tests.sample_doctest', setUp=setUp)
         >>> suite.run(unittest.TestResult())
         <unittest.result.TestResult run=9 errors=0 failures=3>
...
def test_DocFileSuite():
    """We can test tests found in text files using a DocFileSuite.
       ...
       specify a different relative location.
         ...
         >>> import unittest # doctest2-specific modifications to package
         >>> suite = doctest.DocFileSuite('test_doctest.txt',
         ...                              'test_doctest2.txt',
         ...                              'test_doctest4.txt',
         ...                              package='tests')
       ...
       provided.
         ...
         ...     added_loader = True
         >>> try: # doctest2-specific modifications to package
         ...     suite = doctest.DocFileSuite('test_doctest.txt',
         ...                                  'test_doctest2.txt',
         ...                                  'test_doctest4.txt',
         ...                                  package='tests')
...
debugging):
    ...
    ... # doctest2-specific modification below (not doctest.):
